({
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                if (responseString.indexOf("Error - ") == -1 && responseString.indexOf("invalid service resource") == -1) {
                    this.applyChangedServiceResource(component, event, false, responseString)
                } else {
    ...
    applyChangedServiceResource: function(component, event, isNewServiceResource, serviceResource) {

        var effectiveDate = component.get("v.effectiveDate");
        var serviceWrapper = component.get("v.productWrapper");
        var action = component.get("c.changeServiceResource");

        action.setParams({
            productServiceWrapper: JSON.stringify(serviceWrapper),
            serviceResourceToBeApplied: serviceResource,
            isNewServiceResource: isNewServiceResource,
            effectiveDate: JSON.stringify(effectiveDate),
            parentServiceEid: component.get("v.serviceEid")
        });
